...
def upload_to_gcs(local_base_dir: str, bucket_name: str, gcs_prefix: str) -> None:
    """
    Uploads files from local directory to a Google Cloud Storage bucket.

    Parameters:
        local_base_dir (str): Path to upload files from
        bucket_name (str): Google Cloud Storage Bucket to upload files to
        gcs_prefix (str): Blob prefix to store the files to in Google Cloud Storage Bucket
...
def fetch_metadata_from_gcs(schema: List[str], year: int, bucket_name: str) -> Dict[str, Dict]:
    """
    Fetch metadata from Google Cloud Storage bucket for each of the schemas

    Parameters:
        schema (list): List of schemas to fetch metadata
        year (int): Year of schema
        bucket (int) : Google Cloud Storage Bucket to fetch from
    
    Returns:
        dict (str, dict): Metadata of each schema for the specified year
    """
    client = storage.Client()
    bucket = client.bucket(bucket_name)
    metadata = {}
...
@benchmark
def process_batch(batch: List, metrics: Metrics, bucket: str) -> None:
    """
    Process batches of partitions by downloading, extracting, validating, converting to Parquet,
    updating metadata, uploading to GCS, and publishing metrics.
    ...
    - Uploads Parquet files and metadata to Google Cloud Storage.
    - Manages temporary directories and publishes processing metrics.

    Parameters:
        batch (list):
            A list where each element is a list of partition dictionaries to be processed together.
        metrics (Metrics):
            An instance of the Metrics class used to record and publish processing metrics.
        bucket (str):
            The name of the Google Cloud Storage bucket where processed files and metadata are uploaded.
